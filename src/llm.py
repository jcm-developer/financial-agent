...
**Nor through the `claude` command on the subscription, which was tried and
withdrawn** (F9.35, 2026-09-26). It worked, but Anthropic reserves the OAuth
token of a Pro/Max plan for ordinary use of Claude Code, and asks anyone building
a service on Claude's capabilities to use an API key. An unattended agent that
calls the CLI with its tools switched off and its own system prompt is the second
thing, not the first, and an API key breaks the 0 € premise. So Claude stays out.

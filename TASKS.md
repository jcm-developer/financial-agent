...
- [x] **F4.24** **Pantalla «Base de datos»: el esquema, lo que pesa cada tabla y qué apunta a
      qué.** Pedido y hecho el 2026-09-25. `GET /api/database/schema` devuelve tablas,
      columnas, claves foráneas con su `on delete`, filas y **tamaño real en disco**, datos
      e índices por separado, medido con `dbstat`. La pantalla, global y fuera del perfil,
      tiene tres vistas: el **diagrama de relaciones** en SVG, colocado por profundidad
      para que cada línea vaya de la tabla hija a la que apunta —leído de derecha a
      izquierda es lo que se lleva un `delete`—; la **tabla de pesos** con su barra; y la
      de relaciones fila a fila, que es la vista de tabla que DESIGN.md exige a toda
      gráfica. Cada caja enseña solo las claves (PK y FK): `agent_settings` tiene 51
      columnas, y entera sería una caja de tres pantallas con dos líneas. Las columnas
      completas van debajo, plegadas.

      ⚠️ **`dbstat` es una opción de compilación**: el SQLite del contenedor la trae y el
      del Python de Windows no. Sin ella el tamaño sale como «—» y la pantalla lo dice, en
      vez de estimarlo con el número de filas. Y el tamaño del fichero sale del sistema de
      ficheros y no de `pragma page_count`, porque el guardián de la API solo deja los
      pragmas que la capa de datos necesita y un tamaño no justifica ampliar esa lista.
...
- [ ] **F9.1** Modelo premium cuando el experimento dé señales.
      ...
      - **Claude: no.** F9.35 lo hizo por el comando `claude` y la suscripción y se retiró
        el mismo día: el token de un plan Pro/Max no es para esto, y la API se factura.
...
- [x] **F9.4** **Sistema de noticias como entrada del analista.** ~~Depende de F9.7~~ **La
      ...
      **Diseño propuesto, pendiente de confirmar:**
      - **Las recoge el sistema, no el modelo.** Google News RSS por nombre de empresa y en
        el idioma de su bolsa, Yahoo de respaldo. Hace falta un **nombre de búsqueda curado
        por símbolo** en el universo: el de `.info` llega truncado en 7 de los 89.
      - **Se guardan con fecha y fuente** en una tabla propia, con el id del ciclo que los
        leyó. Es lo que F9.7 pedía para poder interpretar el histórico después, y lo que
        permite a F9.2 no usar un titular publicado después de la barra.
      - **El modelo los recibe con un id** (`N3: «…», El Economista, 24-09`) y solo puede
        citarlos por ese id, en un campo nuevo de la propuesta. Lo que cite fuera de la lista
        sigue siendo una alucinación visible, así que **la regla de honestidad del prompt se
        reescribe en vez de levantarse**.
      - **Filtro de ruido antes del prompt:** páginas de cotización, foros y homónimos
        (medido: «VEOLIA Cours Action VIE», «Foro Logista», Almirall en la búsqueda de Rovi).
      - **En un perfil nuevo**, no en `eu-05`: comparar «con» contra «sin» pide dos perfiles,
        como dejó escrito F9.7.

      **Descartado: la búsqueda web propia de los modelos GPT-6.** Solo existe con un
      proveedor, así que no se puede comparar contra un perfil de NIM; cada ciclo lee cosas
      distintas; y el modelo elige qué lee, que es invitarlo a buscar lo que le da la razón.
...
- [ ] **F9.7** ⚠️ **Spike: ¿se pueden conseguir noticias y fundamentales, y a qué precio?**
      ...
      **Medido el 2026-09-25, la mitad de noticias.** Los fundamentales siguen sin medir.

      1. **Yahoo no sirve como fuente única, y falla justo donde temía la pregunta 1.**
         `get_news(count=20)` sobre los 89: **32 sin ninguna noticia en 7 días** (19 en 30
         días), mediana 2 por símbolo y semana. Por bolsa, **Madrid 23 de 35 a cero**, con
         mediana 0; Fráncfort 1 de 16, París 5 de 22. Es el sesgo que se temía, medido:
         el modelo vería contexto de ASML (20 en la semana) y silencio de ACS o CaixaBank.
         Y lo que llega es sobre todo prensa estadounidense de agregador —Zacks, Insider
         Monkey, Simply Wall St., GuruFocus—, con titulares tipo «European Dividend Stocks
         To Consider» que no dicen nada del valor. Solo titular, sin cuerpo.
      2. **Google News RSS, por nombre de empresa y en el idioma de su bolsa, cubre los
         89.** Sobre los 32 que Yahoo deja a cero, 25 tienen noticias a la primera; los 7
         restantes eran del nombre que devuelve `.info` («CORPORACI...N ACCIONA ENERG...A»,
         «DEUTSCHE BOERSE N»), y con el nombre escrito a mano salen todos: Acciona Energía
         46, Indra 100, Logista 16, Deutsche Börse 61. Las fuentes cambian de categoría:
         Expansión, El Economista, Europa Press, Boursorama, Le Figaro Bourse. **Tiene ruido
         propio** y hay que filtrarlo: páginas de cotización («VEOLIA Cours Action VIE»),
         foros («Foro Logista»), y homónimos (la búsqueda de Rovi trajo una ficha de
         Almirall). Consecuencia: el universo necesita **un nombre de búsqueda por
         símbolo, curado**, igual que FE.2 verificó los sufijos uno a uno.
      3. **Coste: 0 €** con las dos. No hay clave ni cuota publicada, y por eso mismo
         ninguna garantía: es otra dependencia que puede cambiar sin avisar, como Yahoo.
      4. **Peticiones:** una por símbolo y ciclo son 89-90 más por ciclo desde la IP
         doméstica, a otro dominio distinto de Yahoo, así que R2 no se suma. Con un
         segundo de pausa son ~1,5 min de ciclo, frente a los ~20 del modelo.

      **Lo que queda por decidir para F9.4** es el diseño de la regla de honestidad: si el
      modelo recibe los titulares **con un id** y solo puede citarlos por ese id, lo que
      cite fuera de la lista sigue siendo una alucinación visible, y la garantía de hoy
      se conserva en vez de perderse.
...
- [x] **F9.19** **El ciclo asigna el capital por orden de llegada, no eligiendo entre lo que ha
      ...
      **Pedida de nuevo el 2026-09-25**, en la lista de mejoras. Sigue detrás de F9.20 por la
      misma razón: con `buy` en 79 de 93, ordenar por convicción aún no separa nada.

      ✅ **Hecho el 2026-09-26, antes del primer ciclo del experimento de seis meses**,
      porque después habría partido el histórico. Las dos objeciones de arriba ya no
      aguantaban:
      - **La convicción ya no es una constante.** Con Sonnet 5, en el ciclo de prueba de
        F9.35, salió de 35 a 58 en 20 respuestas, y las cuatro `buy` entre 56 y 58. F9.20
        sigue abierta —calibrar no es lo mismo que variar—, pero ordenar ya ordena algo.
      - **El coste de precio no existe en este simulador**: todas las órdenes de un ciclo
        salen a la apertura de la misma barra, fijada al bajar los datos. Daba igual
        mandarlas a las 10:21 o a las 10:40. Con F9.3 volverá a importar, y está dicho en
        el código.

      **Lo que quedó:** el paso de entradas analiza los veinte candidatos y después reparte
      entre las `buy` **de más a menos convicción**, con el orden del screener como
      desempate (el `sorted` es estable). Las que se quedan sin plaza se registran con la
      regla nueva **`entry_cap`**, «sin plaza en el ciclo», para que la pantalla de Riesgo y
      F9.27 vean por qué una `buy` no se ejecutó. Con la cartera llena no se pregunta nada.

      **Descartado:** ordenar por sigmas de recorrido prometido —la convicción es la cifra
      con la que el propio modelo dice cuánto se fía, y la que F9.27 calibrará— y
      combinar las dos en una puntuación, que sería inventar una fórmula sin datos.

      **Dos consecuencias que hay que asumir:**
      - **Más llamadas**: con plazas libres se analizan siempre los veinte, en vez de cortar
        al llenar el tope. Es lo que ya medía el ciclo de prueba de F9.35 (20 llamadas).
      - **Una parada durante el análisis ya no compra nada**: antes el primer candidato ya
        se había ejecutado. Test y EXPERIMENT.md actualizados.
...
- [x] **F9.25** **El trailing stop no tenía suelo, y en un ciclo dejó la cartera corriendo a
      1 ATR cuando el perfil dice 3.** Abierta y cerrada el 2026-08-27, al revisar el primer
      ciclo bueno con `nemotron-3-super`.

      El ciclo de las 10:53 CEST subió el stop de seis de las nueve posiciones abiertas, y
      cada línea del log parecía razonable por separado —«CABK.MC: stop elevado de 11.95 a
      12.95 por sugerencia del analista»—. Juntas, esto es lo que quedó:

      | símbolo | precio | stop | dist. | en ATR |
      |---|---|---|---|---|
      | **UCG.MI** | 84,89 | **85,00** | **−0,13 %** | **−0,08** |
      | G.MI | 43,79 | 43,50 | 0,66 % | 0,48 |
      | NDA-FI.HE | 17,40 | 17,20 | 1,15 % | 0,88 |
      | CABK.MC | 13,18 | 12,95 | 1,75 % | 1,02 |
      | DHL.DE | 56,32 | 55,30 | 1,81 % | 1,16 |
      | AENA.MC | 27,04 | 26,50 | 2,00 % | 1,20 |
      | IDR.MC *(nueva, stop del motor)* | 60,92 | 55,44 | 8,99 % | 3,03 |
      | LOG.MC *(sin tocar)* | 35,64 | 33,50 | 6,00 % | 4,40 |

      UCG.MI tenía el stop **por encima del precio vivo**: el ciclo siguiente la habría
      cerrado por stop. Y las otras cinco quedaban a menos de 1,2 ATR en un experimento con
      horizonte de 45 días y objetivos del +12 %, o sea posiciones que no salen por tesis rota
      sino por el ruido del martes.

      **La causa es una asimetría del código y no el modelo.** En una entrada, `risk.py`
      acepta el stop del analista **solo si es más ancho** que el de `stop_atr_multiple` —el
      `llm_wider` de [src/risk.py](src/risk.py), con su «never the other way round»— porque la
      distancia al stop *es* la unidad de riesgo de este sistema. En una salida,
      `_maybe_raise_stop` aceptaba cualquiera **más estrecho**, con la única condición de estar
      bajo el precio, y sin distancia mínima. «Solo más cerca» se leyó como «sin riesgo
      añadido», y no es lo mismo: quien acabó fijando el parámetro de riesgo fue el modelo, que
      es justo lo que la premisa del proyecto no permite.

      **Lo que se hizo:** el suelo es `stop_atr_multiple * TRAILING_STOP_FLOOR_FRACTION`, con
      la fracción a 0,5 — la mitad de la distancia inicial del propio perfil. La sugerencia se
      sigue honrando, pero recortada al suelo, y **el recorte se registra con las dos cifras**
      («el analista pedia el stop en 12.95, a 0.55 ATR del precio; se sube solo a 12.80, el
      suelo de 1.50 ATR»): un stop que acaba en un sitio distinto del que pidió el analista, en
      silencio, es exactamente lo que tardó un ciclo entero en verse.

      **Es una fracción y no un número absoluto de ATR** porque `stop_atr_multiple` es del
      perfil: un conservador con stop a 3 ATR tiene suelo en 1,5 y un agresivo con 1,5 lo tiene
      en 0,75, que es la misma decisión escrita en la unidad de cada uno.

      **Lo que se descartó:**
      - **Suelo en 2 ATR.** Congela el ratchet: con el stop inicial en 3 ATR, casi ninguna
        posición joven puede subirlo, así que la mitad útil del mecanismo desaparece para
        arreglar la otra.
      - **Suelo en una sigma del horizonte restante**, que es lo coherente con F9.16 y
        conceptualmente lo correcto. Necesita saber cuánto horizonte queda, que hoy no viaja
        hasta aquí, y el suelo en ATR ya corrige el caso medido. Queda como la salida si 1,5
        ATR resulta ser demasiado estrecho.
      - **Prohibirle al analista mover el stop.** Era la respuesta fácil y quita algo que sí
        vale: cinco de los seis stops de hoy, recortados al suelo, siguen estando **por encima**
        del que tenía la posición antes del ciclo.

      ⚠️ **Los seis stops del ciclo ya estaban escritos, así que se recolocaron a mano.** Con la
      misma fórmula, contra el precio vivo del momento y sin bajar nunca del stop anterior al
      ciclo, que salió de las líneas del propio log porque `positions` **no guarda historial de
      sus niveles**:

      | símbolo | stop del ciclo | recolocado | antes del ciclo |
      |---|---|---|---|
      | AENA.MC | 26,50 | 26,32 | 26,00 |
      | CABK.MC | 12,95 | 12,80 | 11,95 |
      | DHL.DE | 55,30 | 55,08 | 53,50 |
      | G.MI | 43,50 | 42,77 | 42,60 |
      | NDA-FI.HE | 17,20 | 17,09 | 16,90 |
      | UCG.MI | 85,00 | 82,45 | 81,50 |

      Es una escritura directa en `positions` desde el contenedor `bot`, fuera de un ciclo y
      por tanto **sin rastro en el histórico más que esta tabla**. La API no podría haberla
      hecho —`positions` no está entre las cinco tablas escribibles de
      [api/guard.py](api/guard.py)— y eso es correcto: lo que hace falta no es abrirla, es que
      esto no vuelva a hacer falta.

      ⚠️ **Un stop cerca del precio no siempre es este fallo, y confundirlo es el error
      fácil.** BNP.PA está a 0,58 ATR y no se tocó: su stop son los 3 ATR de su entrada
      (106,93 → 100,50) y lo que se ha acercado es el precio, cayendo. Eso es una posición
      perdiendo camino al stop, que es el diseño funcionando. El suelo es del **momento de
      subirlo**, no un invariante permanente de la cartera.
...
- [ ] **F9.20** **El modelo contesta con plantilla, y tres de sus cuatro campos son constantes.**
      ...
      **Sigue igual con `nemotron-3-super`, medido el 2026-09-25 sobre toda la tabla
      `decisions`:** entradas, **`buy` en 79 de 93**; salidas, **`hold` en 870 de 897**. Tres
      modelos distintos y el mismo sesgo, lo que apunta cada vez más al prompt o a la tarea y
      menos a un modelo concreto. **Recalibrar** es probar, en este orden y de uno en uno:
      la temperatura (punto 1), un modelo de otra categoría (F9.26), y un prompt que pida
      primero el caso bajista y luego el alcista, o que puntúe contra una base de referencia
      explícita («¿es esto mejor que el candidato medio del screener?»). La vara con la que
      se mide si mejoró es **F9.27**: la convicción está calibrada si los de 70 aciertan más
      que los de 55, y hoy eso no se puede calcular.

- [ ] **F9.26** **Revisar `gpt-6-sol` como modelo del experimento, y lo que F9.1 da por hecho.**
      Pedido el 2026-09-25. Concreta F9.1 para GPT.

      **Por qué Sol y no Luna ni Astra**, con el catálogo de OpenAI del 2026-09-25:

      | Modelo | Entrada / salida por M tokens | Para qué se vende | Estimado al mes aquí |
      |---|---|---|---|
      | `gpt-6-luna` | 0,10 $ / 0,50 $ | Volumen: resumir, extraer, clasificar | ~2–4 $ |
      | `gpt-6-sol` | 2 $ / 10 $ | Tareas complejas, «la mitad de errores que su predecesor» | ~30–70 $ |
      | `gpt-6-astra` | 10 $ / 50 $ | Código y uso del ordenador | ~150–360 $ |

      El estimado sale de los tokens reales de `decisions` (entrada ~2.750 / ~1.300 de salida
      con `nemotron-3-super`), 50–120 llamadas al día y ~1.000 tokens más por noticias. Lo que
      falla hoy es juzgar sin certeza y calibrar (F9.20), que es justo lo que Luna no promete.
      Luna sirve como **segundo perfil barato**, no como el principal.

      ⚠️ **«GPT: ya se puede, cero código pendiente» (F9.1) probablemente ya no es cierto.**
      [src/llm.py](src/llm.py) manda `max_tokens: 1600` y `temperature: 0.2`. Los modelos de
      razonamiento de OpenAI suelen pedir `max_completion_tokens`, un `reasoning_effort` y la
      temperatura por defecto, y 1.600 tokens se los come el razonamiento antes del JSON, igual
      que le pasó a Nemotron Ultra en F9.23. **Sin verificar**: hace falta una llamada con la
      clave de OpenAI. Si salen bien, aprovechar las salidas con esquema JSON, que cierran F9.24
      por el otro lado.

      **Dos consecuencias que hay que asumir:**
      - **Rompe la premisa de 0 €**, que era el aviso de F9.1.
      - **Corte de conocimiento: 2026-04-20.** No afecta al vivo, pero `replay_analyst.py` y
        F9.2 sobre fechas anteriores le preguntarían al modelo por un pasado que ya conoce.

      La ventaja que no sale en ningún benchmark: **el histórico dejaría de partirse cada
      semana** por un modelo que NVIDIA retira o deja colgado (cuatro cortes en doce días,
      F9.22–F9.23.1).

      **Verificado el 2026-09-25, y el aviso era cierto.** Con el perfil `eu-sol-base`
      pausado y el prompt de entrada real sobre SAN.MC, ASML.AS e ITX.MC —sin ciclo y sin
      escribir en la base—, **las 3 llamadas murieron con un 400**: `max_tokens` no se
      acepta y hay que mandar `max_completion_tokens`. Y había un segundo fallo detrás: el
      error no nombraba ninguno de los dos extras negociables, así que el respaldo a ciegas
      de `_disable_rejected_option` **apagaba el modo JSON**, que no era el problema.

      Arreglado en [src/llm.py](src/llm.py) **negociando, no por proveedor**: el nombre del
      techo y la temperatura se cambian cuando el error los nombra, sin gastar intento y
      para el resto de la sesión, igual que `response_format` y `stream_options`. NIM sigue
      recibiendo `max_tokens`. Descartado cablearlo a `provider == "openai"`: OpenAI no pide
      lo mismo a todos sus modelos, y el nombre del modelo no dice cuál quiere.

      **Tras el arreglo, 3 de 3**, JSON válido con los nueve campos, 10–16 s por llamada,
      **~2.300 tokens de entrada y 366–521 de salida**, o sea ~0,009 $ por análisis de
      entrada: menos de lo estimado arriba, porque Sol razona bastante menos que Nemotron
      (~1.300 de salida). El techo de 1.600 va holgado. Las tres, `hold` con tesis que citan
      las cifras del prompt; tres son pocas para decir nada de la calibración (F9.20).

      **Y un segundo perfil con Luna, en paralelo (2026-09-25).** `eu-luna-base` es un
      duplicado exacto de `eu-sol-base` con `llm_model = gpt-6-luna`, para que los dos
      corran sobre los mismos candidatos y las mismas barras cada día. Medido con un
      ciclo de prueba sobre una copia de la base: 5 de 5 respuestas con JSON válido,
      6–11 s, noticias citadas en 4 de 5, ~3.100 tokens de entrada y ~750 de salida,
      o sea **~0,0007 $ por llamada**, diecisiete veces menos que Sol. ⚠️ El
      planificador lanza los ciclos **en serie**, no a la vez: con `1h` los dos ven la
      misma barra mientras el segundo termine antes de que llegue la siguiente (a las
      11:15 para un ciclo de las 10:20), y cada uno pide sus propias noticias, así que
      los titulares pueden diferir en los minutos que los separan. La comparación
      buena no es el P&L —cinco posiciones en seis meses no separan dos modelos— sino
      decisión a decisión sobre el mismo candidato el mismo día, que es **F9.27**.

      ⚠️ **Sol no acepta `temperature=0.2`** y corre con la suya por defecto. Se registra
      como WARNING en el log del ciclo, pero `cycles.settings_json` seguirá diciendo 0,2
      mientras el perfil lo diga: hay que poner `llm_temperature` a 1 en el perfil para que
      la copia no mienta. **No se ha probado el `reasoning_effort`**: va con el de defecto.

- [ ] **F9.27** **Medir si cada decisión acertó, `hold` incluidos.** Pedido el 2026-09-25.

      Hoy solo se juzgan las operaciones ejecutadas, así que el P&L mezcla el acierto del
      modelo con lo que hicieron el Risk Manager, el tope de entradas y el orden de llegada.
      Y los `hold` —870 de 897 salidas— no se evalúan nunca: no se sabe si aguantar fue bueno.

      **La idea:** para cada fila de `decisions`, apuntar el precio a `horizon_days` (y a
      plazos intermedios, 5 y 20 sesiones) y si tocó antes el stop o el objetivo propuestos.
      Con eso salen la tasa de acierto por acción, la **calibración de la convicción** (los
      de 70 deberían acertar más que los de 55) y el valor de los `hold` de salida.

      - **Es un cálculo, no un registro nuevo del ciclo**: sale de `decisions` y de
        `bar_cache`, así que se puede calcular a posteriori para todo el histórico.
      - ⚠️ **Hay que separar los cortes del histórico** (F9.9, F9.16, F9.22, F9.23): calibrar
        un modelo con las decisiones de otro es mezclar dos experimentos.

- [ ] **F9.28** **Un perfil de control sin modelo, para ver si el modelo aporta algo.**
      Pedido el 2026-09-25.

      Es la pregunta de fondo del proyecto y nada la responde hoy. F5.7 mide si el screener
      aporta; esto mide si **el analista** aporta. Un perfil que compra lo que el screener
      pone arriba, con el mismo Risk Manager, los mismos stops, las mismas comisiones y el
      mismo tope de entradas, y sin llamar a ningún modelo. Si el perfil con modelo no le gana
      después de comisiones, el modelo está añadiendo coste y no criterio.

      - Tiene que ser **un modo del analista** (`analyst_mode = rules`, al estilo de
        `screener_mode`), no un fork del ciclo, para que la única diferencia sea esa.
      - Sale gratis: no gasta cuota, así que puede correr siempre en paralelo al perfil que se
        mida.

- [ ] **F9.29** **Comparar el resultado con el Euro Stoxx 50.** Pedido el 2026-09-25.

      `Market.benchmark` existe y la columna `benchmark` del perfil no la lee nadie (F10.11).
      Sin índice de referencia un +5 % no se puede interpretar: en un mes en que el mercado
      hace +8 es perder. La Analítica necesita la curva del índice sobre las mismas fechas,
      rebasada al capital inicial, y el exceso de rentabilidad. ⚠️ **Índice de rentabilidad
      total o de precio**: el de precio no cobra dividendos, y el universo tiene muchos
      valores españoles con dividendos altos, así que eso inclina la comparación.

- [ ] **F9.30** **Contexto de mercado y macroeconomía: que el analista sepa qué está pasando en
      el mundo.** Pedido el 2026-09-25 («si hay una guerra, a lo mejor no le interesa comprar
      X»). Complementa F9.4, que es por empresa.

      Hoy cada llamada ve un activo aislado: no sabe si el índice lleva un −10 %, si el BCE
      sube tipos mañana o si el Brent se ha disparado. Propuesta en tres capas, de más
      barata a menos:

      1. **Régimen de mercado, determinista y gratis.** Un bloque calculado una vez por ciclo:
         tendencia y caída desde máximos de los índices, volatilidad implícita (VSTOXX),
         EUR/USD, Bund a 10 años, Brent, oro y fuerza relativa por sector. Los datos salen de
         Yahoo, pero **hay que verificar que los sirve** símbolo a símbolo, como FE.2.
      2. **Resumen de mercado en una llamada por ciclo.** Titulares generales de macro y
         geopolítica (BCE, inflación, conflictos, materias primas) resumidos por el modelo
         **una vez**, guardados en el ciclo con los ids de los titulares que usó, y pasados a
         todas las llamadas por símbolo. Cuesta una llamada, no veinte, y es la capa que
         responde a lo de la guerra.
      3. **Calendario.** Fechas de resultados por símbolo, reuniones del BCE y datos de IPC.
         Una regla determinista en `risk.py` —no abrir a menos de N sesiones de resultados—
         vale más que decírselo al modelo, y no rompe la premisa.

      **Requisito previo: el sector de cada símbolo**, que falta desde FE.12. Sin él, «hay
      guerra» no se puede convertir en «defensa sí, aerolíneas no», y tampoco se puede aplicar
      el tope por sector.

      ⚠️ **La misma trampa que F9.4, y más grande:** un resumen de mercado es texto generado,
      así que **tiene que guardarse tal cual** en el ciclo. Si no, nadie sabrá después qué
      contexto llevó al modelo a comprar.

- [ ] **F9.31** ~~⚠️ **Divisa: el tipo de cambio del momento de cada operación, y reabrir D8.**~~
      Pedido el 2026-09-25. **Aparcada el mismo día, a petición propia**: D8 sigue en pie y
      ningún perfil sale de la zona euro. Se conserva el análisis de abajo para no tener que
      rehacerlo si algún día se abre un perfil en otra divisa. El EUR/USD **como contexto**
      sigue dentro de F9.30, porque ahí no se convierte nada.

      **Contradice una decisión tomada**, y por eso va con aviso: D8 dejó escrito que un
      perfil cubre una sola bolsa y que el proyecto **no convierte divisa en ningún sitio**.
      Hoy eso es cierto y basta, porque cada cartera está en la moneda de su mercado. Hace
      falta convertir en cuanto una cuenta en euros compre en otra divisa: Estados Unidos,
      Londres, Zúrich o Estocolmo.

      **Qué supondría:**
      - Una **divisa base por cartera**, distinta de la del mercado.
      - El **tipo de cambio en cada orden, guardado en la fila**, tomado en el mismo instante
        que el `fill_price`, con el mismo reloj (sección 5 de EXPERIMENT.md). Un tipo de otra
        hora mezclaría dos precios de momentos distintos.
      - **La comisión de cambio del banco**, que es un coste aparte de la de la orden y que
        el Risk Manager tendría que tener en cuenta, igual que en F9.9.
      - **El P&L partido en dos**: lo que ganó el valor y lo que ganó o perdió la divisa. Si
        no, un +5 % en dólares con el euro subiendo un 5 % se lee como un acierto del modelo.
      - Londres cotiza en **peniques**, que es otra unidad antes de ser otra divisa.
...
- [x] **F9.33** **El alta de un perfil europeo nace con los valores de Estados Unidos, y no
      ...
      El formulario de alta solo pregunta mercado, capital, deslizadores y modelo. Todo lo
      demás sale de los defaults de `schema.sql`, que son los del diseño original para la
      bolsa americana con barras diarias. Tres de ellos contradicen lo que ya se decidió:

      | Columna | Nace con | Lo decidido | Qué pasa |
      |---|---|---|---|
      | `lookback_days` | 200 | 400 (F9.14) | 200 días naturales son ~138 sesiones: `sma_200`, `high_52w`, `low_52w` y sus distancias salen **null**, y el analista juzga con un tercio de los indicadores en blanco sin que nada avise |
      | `max_new_positions_per_cycle` | 0 (sin tope) | 2 (F9.18) | El primer ciclo se come la caja en los primeros candidatos |
      | `horizon_days` | 10 | 45 (decisión nº 9) | El objetivo se mide a otra escala |

      **La salida es la de FE.11**: que `create_market_profile` los rellene desde el
      mercado o desde una tabla de valores de alta, como ya hace con el suelo de liquidez.
      El horizonte y la frecuencia deberían estar además **en el formulario**, porque definen
      el experimento tanto como los deslizadores. En `eu-sol-base` se pusieron a mano.

      **Y dos cosas pequeñas de la misma sesión:**
      - **El botón dice «Crear y activar», y activa.** El docstring de
        [NewProfileForm.tsx](app/src/components/NewProfileForm.tsx) explica que el perfil
        nace en borrador, y es verdad, pero dura lo que dura el clic. No hay forma de crear
        un perfil para revisarlo antes de que el planificador lo recoja.
...
- [ ] **F9.32** **Ideas del 2026-09-25, sin decidir todavía:**
      - **Revisión de cada posición al cerrarla.** Una llamada que compare la tesis de entrada
        ...
        ciclos siguientes contaminaría la comparación entre perfiles.
      - **Preguntar dos veces lo mismo.** Con Luna es casi gratis y mide cuánto ruido tiene
        el propio modelo: si la misma pregunta da `buy` una vez y `hold` la otra, la
        convicción no significa nada.
      - **Correlación de la cartera aplicada, no solo pedida.** El prompt dice «evita
        concentrar en activos muy correlacionados» y nada lo comprueba. Una regla en
        `risk.py` con la correlación de 60 sesiones contra lo que ya hay abierto.
      - **Dividendos: el simulador no los abona, comprobado.** Las barras llegan sin ajustar
        (`auto_adjust=False`, `actions=False` en `bar_cache.py`, `ingest.py` y
        `market_data.py`) y nada en `src/` apunta un dividendo. El día ex-dividendo el precio
        cae el importe del dividendo y la cartera se lo come entero: el P&L sale por debajo
        del real, y en valores españoles la caída puede perforar un stop sin que la tesis
        haya cambiado. También inclina F9.29 si se compara contra un índice de rentabilidad
        total.
...
- [x] **F9.35** **Un tercer perfil con Claude Sonnet 5, por la suscripción y no por la API.** — **Retirado el 2026-09-26.**
      Pedido el 2026-09-25. `eu-claude-base` es un duplicado exacto de `eu-sol-base` con
      `llm_provider = anthropic` y `llm_model = claude-sonnet-5`, activo desde el
      2026-09-26: los tres perfiles corren sobre los mismos candidatos cada día y solo
      cambia el modelo.

      **Por qué Sonnet 5 y no Opus 5.5:** Sonnet cuesta por token lo mismo que Sol
      (2 $ / 10 $ por M), así que la comparación es entre modelos del mismo escalón. Con la
      suscripción Opus también saldría gratis, pero la pregunta pasaría a ser «el mejor
      Claude contra el GPT intermedio». Queda como posible cuarto perfil, no como tercero.

      **Por qué el comando `claude` y no el SDK** (que era lo que proponía F9.1): la API se
      factura por token, y el token de `claude setup-token` solo se puede usar a través de
      Claude Code — sacarlo al SDK no está permitido. La documentación contempla ese token
      expresamente para scripts. [src/claude_cli.py](src/claude_cli.py) lanza
      `claude -p` en un solo turno y sin herramientas, y `LLMClient` le pasa la llamada
      cuando el proveedor tiene `transport="cli"`: ni el analista ni el ciclo notan la
      diferencia. Se reutiliza el valor `anthropic` que el `CHECK` del esquema admitía desde
      F1.2; un `claude-code` nuevo habría obligado a reconstruir `agent_settings`.

      **La clave decide si se paga, no el proveedor:** una `sk-ant-oat…` sale como
      `CLAUDE_CODE_OAUTH_TOKEN` (suscripción) y cualquier otra como `ANTHROPIC_API_KEY`
      (facturada). ⚠️ El CLI **prefiere la clave de API al token**, así que al proceso hijo
      se le quitan las `ANTHROPIC_*` que herede: la sesión desde la que se implementó
      tenía una, y habría convertido cada llamada en facturada sin dar ningún error.

      **Tres hallazgos de la medición que no estaban en el plan:**
      - **Los flags separan 186 tokens de entrada de 8.389**, con el mismo prompt de una
        línea: lanzado desde el repo, el CLI carga por su cuenta CLAUDE.md, los ajustes y los
        servidores MCP y los cobra como entrada en cada llamada. Por eso corre en un
        directorio vacío, con `--setting-sources ""`, `--strict-mcp-config`, `--tools ""` y
        `--exclude-dynamic-system-prompt-sections`. Sin esto, el perfil habría gastado
        cuatro veces lo estimado.
      - **Sin `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`, cada llamada hacía otra a Haiku**
        de ~500 tokens.
      - **`--json-schema` se descarta:** valida la salida (cerraría F9.24 en este camino),
        pero funciona con una herramienta interna, así que cada llamada tarda dos turnos y
        paga el prompt dos veces. Sin él, Sonnet devuelve JSON limpio en un turno.

      **Medido con un ciclo `--dry-run` completo** sobre una copia de la base, en un
      contenedor de la imagen nueva (con `bar_interval` a `1d` en la copia, porque con `1h`
      el ciclo se omite con la bolsa cerrada; los indicadores del analista son diarios
      desde F9.14, así que el prompt es el mismo): **20 de 20 respuestas con JSON válido**,
      4 `buy` y 16 `hold` con convicción de 35 a 58 —no la plantilla de F9.20—, ~4.900
      tokens de entrada y ~1.800 de salida por llamada (98.000 y 36.000 en el ciclo),
      17–44 s por llamada y ~10 min de ciclo. En la API serían ~0,56 $ por ciclo, unos
      12 $ al mes; por la suscripción no se factura, sale del cupo compartido.
      La entrada es mayor que la de Luna (~3.100) con el mismo prompt, porque el
      tokenizador de Claude cuenta más, y la salida incluye el razonamiento.

      **Docker:** la imagen instala Claude Code **2.1.185, fijado**, con el autoactualizador
      apagado: un CLI que se actualiza solo cambiaría sus flags debajo del experimento, y
      son los flags los que fijan el consumo. El token vive en el perfil (`llm_api_key`),
      como las claves de OpenAI, no en el `.env`.

      **Consecuencias que hay que asumir:**
      - ⚠️ **El cupo es el mismo que el del uso interactivo.** Si una mañana se agota la
        ventana de 5 horas antes de las 10:20, el ciclo de este perfil falla y queda un
        hueco. Un cupo agotado no se reintenta: dura horas y solo quemaría la espera.
      - Ni la temperatura ni el techo de tokens llegan al modelo (el CLI no los acepta);
        el perfil lleva `llm_temperature` 1 para que `cycles.settings_json` no mienta. El
      ...
      **Cambiado a Opus 5.5 el 2026-09-26, antes del primer ciclo real.** Pedido tras ver
      la comparación de arriba: el perfil pasa a `claude-opus-5-5` con
      `llm_reasoning_effort = medium` y se renombra a **`eu-opus-base`**. Se asume a
      sabiendas lo que se descartó arriba: la comparación deja de ser del mismo escalón de
      precio y pasa a ser «el mejor Claude contra Sol», así que si gana no dirá si es por
      ser Claude o por ser más grande. Como no había ningún ciclo, no parte ningún histórico.
      - **Claude Code 2.1.185 no conoce Opus 5.5**: responde un 400 que pide la 2.1.280 o
        posterior. La imagen pasa a **2.1.283**, y se comprobó dentro del contenedor que los
        flags de aislamiento siguen valiendo: 256 tokens de entrada fijos y sin llamada a
        Haiku.
      - **Ese 400 se reintentaba**, porque todo error del CLI se daba por pasajero. Ahora un
        4xx que no sea 429 falla a la primera: la misma petición se volvería a rechazar.
      ...
      **Retirado el 2026-09-26, sin llegar a correr ningún ciclo.** Pedido tras una
      auditoría de cómo se usaba la cuenta de Claude. La página legal de Claude Code dice que
      la autenticación OAuth de un plan Pro/Max está pensada para «ordinary use of Claude
      Code», y que quien construya «products or services that interact with Claude's
      capabilities» tiene que usar una clave de API de la Console. Un agente que corre solo
      cada día, llama a `claude -p` con las herramientas apagadas y su propio system prompt,
      y usa el CLI como pasarela de completions, está en el segundo caso aunque el binario sea
      el oficial y el token sea del dueño. Lo que se arriesgaba era la cuenta personal, y
      Anthropic se reserva actuar sin aviso.
      - **Se descarta pasarlo a clave de API**, que el código ya admitía cambiando la celda:
        ~0,56 $ por ciclo con Sonnet y el doble con Opus rompen la premisa de 0 € (F9.1).
      - **Lo que se quita:** el perfil `eu-opus-base` y su cartera vacía,
        `src/claude_cli.py` y sus tests, la fila `anthropic` de `PROVIDERS` —vuelve a decir
        «no implementado todavía»—, «Claude (suscripción)» de los selectores de Ajustes y del
        alta, y Claude Code de la imagen, que vuelve a la de antes de F9.35.
      - **Lo que se queda:** el arreglo del renombrado de carteras, que era un fallo general
        y no de este perfil, y el valor `anthropic` del `CHECK` de `agent_settings`, que ya
        estaba desde F1.2 y quitarlo obligaría a reconstruir la tabla.
      - ⚠️ **El token de `claude setup-token` sigue vivo** hasta que se revoque desde
        claude.ai: borrar el perfil lo quita de la base, no de la cuenta.

- [x] **F9.36** **Los stops se comprueban cada hora, sin modelo, entre ciclo y ciclo.**
      Pedido el 2026-09-26. Con un solo ciclo al día (decisión nº 11), las salidas
      obligatorias solo se miraban a las 10:20: un stop perforado a las 11:00 no se
      vendía hasta el día siguiente. Ahora el planificador lanza `run.py check-stops`
      **cada hora de 11:20 a 17:20**, que es solo el paso 6 del ciclo: sin screener, sin
      modelo, sin entradas y sin subir stops, que son cosas que piden juicio.

      **Por qué esas horas** ([tools/scheduler.py](tools/scheduler.py),
      `stop_check_times`): al minuto del ciclo del perfil, porque es el que se eligió
      para la barra —a :20 ya ha llegado, con el retraso de 15 minutos, la que cerró a
      :00—; solo después del primer ciclo del día, porque antes la última barra completa
      es la de ayer; y solo dentro de la ventana operativa, **convertida de la zona del
      mercado a la del perfil**, que para un perfil americano en Madrid da de 17:20 a
      21:20. Con barras diarias no hay ninguna: el precio no se mueve hasta el cierre.

      **Lo que se decidió:**
      - **No escribe nada si no salta nada.** Siete comprobaciones al día por perfil
        llenarían `cycles` de filas vacías y la curva de capital de puntos sin nada. Si
        salta, queda como un ciclo más —la orden y la posición necesitan un
        `cycle_id`—, con `llm_model` NULL y `cycle_kind: stop_check` en `settings_json`.
      - **Solo baja lo que hay en cartera**: `fetch_positions` en los dos proveedores,
        con las mismas cachés y el mismo `build_snapshot` que el ciclo, así que el
        precio contra el que se mira el stop es el que verá el ciclo siguiente. Pasar
        por `fetch_snapshots` —lo que hace el cierre del experimento— habría
        refrescado y cribado los 89 valores siete veces al día para mirar cinco.
      - **No va al log en vivo de Ciclos**: lo sobrescribiría siete veces al día y
        taparía el del ciclo de las 10:20. Lo que salta queda en el histórico.
      - **Sin columna ni interruptor en el perfil.** Es protección pura, no gasta cuota y
        se aplica igual a los tres perfiles, así que no cambia la comparación; y el
        experimento aún no ha empezado, así que no parte ningún histórico. Si algún
        día hace falta apagarlo, es una columna.

      **Descartado:** más ciclos con modelo —lo que la decisión nº 11 ya descartó— y
      mirar `quotes_live` minuto a minuto, que es F9.3 y rompería la regla de que el
      agente no lee lo que captura el ingestor.

      **Consecuencia que hay que asumir:** la resolución es la de la barra horaria. Un
      stop real lo vigila el broker tick a tick, así que esto sigue siendo más grueso
      que la realidad, pero ya no un día entero. Verificado en el contenedor: el
      planificador anuncia las siete horas en los tres perfiles, y `check-stops` en
      sábado se omite diciendo por qué.
...
10. **Borrón y cuenta nueva.** ✅ **Hecho el 2026-09-25, a petición propia.** El experimento
    del mes (decisión nº 9) ya ha cumplido su función —probar el circuito y ajustar las
    reglas, de F9.14 a F9.25— y las mejoras de F9.4 y F9.26–F9.30 lo harían ininterpretable
    de todas formas. Así que **se borró la base entera y los cinco perfiles**, en vez de
    resetearlos con `tools/reset_experiment.py`: esa herramienta conserva los perfiles, y
    esta vez no había nada que conservar.

    **Lo que se perdió a sabiendas:** las 1.213.743 barras de `bars_1m`, del 2026-08-03 al
    2026-09-25. Yahoo solo sirve ~30 días atrás, así que **F9.2 empieza de cero** con lo que
    el ingestor vaya guardando a partir de ahora. `bar_cache` se vuelve a descargar solo.

    **Copia antes de borrar**, con la API de backup de SQLite y `integrity_check` limpio:
    `data/trading-2026-09-25-pre-reset.db` (405 MB, fuera de git). Tiene los 109 ciclos,
    las 990 decisiones y las barras de minuto, y es la que hay que abrir si alguna medición
    de esta semana (F9.7, F9.20, el coste de F9.26) hay que repetirla.

    ⚠️ **`run.py check` escribe en la base, aunque sea un diagnóstico.** Con la base vacía
